...
class RequestConfig(BaseModel):
    ...
    fulfillment_max_retries: int = Field(3, description="Maximum number of provisioning retry attempts")
    fulfillment_timeout_seconds: int = Field(300, description="Provisioning timeout in seconds")
    fulfillment_batch_size: int = Field(1000, description="Maximum instances per provisioning attempt")
    fulfillment_fallback_template_id: Optional[str] = Field(None, description="Fallback template ID for provisioning")
